...
import { Search, MapPin, Heart, X, Clock, User, ChevronDown, Filter, Star, Calendar, Badge, Stethoscope, Award } from "lucide-react";
...
export default function DoctorSearch() {
  ...
  const generateSuggestions = (searchTerm) => {
    ...
    if (searchTerm.trim() === "") {
      return {
        localities: [...new Set(cityDoctors.map(doc => doc.locality).filter(Boolean))],
        specializations: [...new Set(cityDoctors.map(doc => doc.specialization).filter(Boolean))],
    ...
    const filteredDoctors = cityDoctors.filter(doc =>
      ...
      (doc.specialization && doc.specialization.toLowerCase().includes(lower)) ||
      (doc.fullName && doc.fullName.toLowerCase().includes(lower)) ||
      (doc.clinicName && doc.clinicName.toLowerCase().includes(lower))
    );

    return {
      localities: [...new Set(filteredDoctors.map(doc => doc.locality).filter(Boolean))],
      specializations: [...new Set(filteredDoctors.map(doc => doc.specialization).filter(Boolean))],
  ...
  useEffect(() => {
    ...
    if (selectedFilter) {
      filteredDoctors = filteredDoctors.filter(doc => 
        doc.specialization === selectedFilter
      );
    }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center mb-8">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">
              Find Your Perfect
              <span className="text-emerald-600 ml-2">Doctor</span>
            </h1>
            <p className="text-lg text-gray-600 max-w-2xl mx-auto">
              Search through thousands of verified doctors and book appointments instantly
            </p>
          </div>

          {/* Search Bar */}
          <div className="relative mb-8">
            <div className="bg-white shadow-xl rounded-3xl border border-gray-100 p-3 sm:p-4">
              <div className="flex flex-col lg:flex-row gap-4">
                
                {/* City Selection */}
                <div className="flex items-center px-4 py-2 bg-gray-50 rounded-2xl min-w-0 lg:w-64">
                  <MapPin className="text-emerald-500 mr-3 h-5 w-5 flex-shrink-0" />
                  <select 
                    value={selectedCity}
                    onChange={(e) => handleCityChange(e.target.value)}
                    className="outline-none text-sm font-medium text-gray-700 bg-transparent cursor-pointer flex-1 min-w-0"
                  >
                ...
                <div className="flex-1 relative">
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder={selectedCity ? "Search doctors, specializations, or localities..." : "Select a city to search"}
                      disabled={!selectedCity}
                      className={`w-full pl-12 pr-12 py-3 text-base rounded-2xl border-0 bg-gray-50 ${
                        !selectedCity ? "text-gray-400 cursor-not-allowed" : "text-gray-700 focus:bg-white focus:ring-2 focus:ring-emerald-500"
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <label className="flex items-center">
                        <input 
                          ...
                          className="text-emerald-600 focus:ring-emerald-500"
                        />
                        <span className="ml-2 text-gray-700">In-Person Only</span>
                      </label>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      <button className="px-3 py-1 text-xs bg-white border border-gray-300 rounded-full hover:border-emerald-300 transition-colors">
                        Available Today
                      </button>
                      <button className="px-3 py-1 text-xs bg-white border border-gray-300 rounded-full hover:border-emerald-300 transition-colors">
                        Top Rated
            ...
            {showPopup && selectedCity && (
              <div className="absolute top-full left-0 right-0 bg-white border rounded-lg shadow-lg mt-2 z-50 max-h-96 overflow-y-auto">
                ...
                {suggestions.localities?.length > 0 && (
                  <div className="p-4 border-b">
                    <h4 className="text-sm font-medium text-gray-700 mb-2">Localities</h4>
                    <div className="space-y-1">
                ...
                {suggestions.doctors?.length > 0 && (
                  <div className="p-4">
                    <h4 className="text-sm font-medium text-gray-700 mb-2">Doctors</h4>
                    <div className="space-y-2">
                        ...
                        >
                          <div className="flex items-center">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-gray-500">{doctor.specialization} • {doctor.locality}</p>
                            </div>
                          </div>
                        </button>
                      ))}
              ...
              <div className="flex gap-3 flex-wrap">
                {specializations.slice(0, 8).map((filter) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-full text-sm font-medium border-2 transition-all duration-200 transform hover:scale-105 ${
                ...
                {selectedFilter && (
                  <button
                    ...
                    className="px-4 py-2 rounded-full text-sm font-medium border-2 bg-red-50 text-red-600 border-red-200 hover:bg-red-100 transition-colors"
                  >
              ...
              {!searchQuery && !selectedFilter ? (
                ...
                <p>
                  ...
                  {(searchQuery || selectedFilter) && (
                    ...
                    >
                      Clear {searchQuery && selectedFilter ? 'all' : searchQuery ? 'search' : 'filter'}
                    </button>
                  )}
                </p>
              )}
                  ...
                  <div className="grid gap-6">
                    {results.map((doctor) => (
                      <div
                        ...
                        className="group bg-white rounded-2xl shadow-md hover:shadow-2xl transition-all duration-300 p-6 cursor-pointer border border-gray-100 hover:border-emerald-200 transform hover:-translate-y-1"
                      >
                        <div className="flex flex-col lg:flex-row gap-6">
                          ...
                          <div className="flex-shrink-0">
                            <div className="relative">
                              <div className="w-20 h-20 sm:w-24 sm:h-24 bg-gradient-to-br from-emerald-100 to-emerald-200 rounded-2xl flex items-center justify-center group-hover:scale-105 transition-transform duration-300">
                                {doctor.profileImage ? (
                                  <img 
                                  ...
                                  <User className="h-10 w-10 sm:h-12 sm:w-12 text-emerald-600" />
                                )}
                              </div>
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-start justify-between mb-4">
                              <div className="flex-1">
                                ...
                                <p className="text-emerald-600 font-semibold text-base sm:text-lg mb-3 flex items-center gap-2">
                              ...
                              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                                ...
                                <div className="flex gap-3">
                                  <button
                                    ...
                                    className="flex-1 lg:flex-none bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 text-white px-6 py-3 rounded-xl font-semibold transition-all duration-200 transform hover:scale-105 shadow-lg hover:shadow-xl"
                                  >
                                    Book Appointment
                                  </button>
                                  <button
                                    className="px-4 py-3 border border-emerald-200 text-emerald-600 rounded-xl hover:bg-emerald-50 transition-colors"
                                  >
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500 max-w-md mx-auto">
                    {searchQuery || selectedFilter ? (
                      ...
                      <>No doctors are available in {selectedCity} at the moment.</>
                    )}
                  </p>
                  {(searchQuery || selectedFilter) && (
                    <button
