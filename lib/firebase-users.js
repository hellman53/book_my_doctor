...
export const getTechnicalSupportTickets = async () => {
  try {
    const q = collection(db, "technicalSupport");
    const querySnapshot = await getDocs(q);
    const tickets = [];
...
// Update technical support ticket
export const updateTechnicalSupportTicket = async (ticketId, updates) => {
  try {
    const ticketRef = doc(db, "technicalSupport", ticketId);
